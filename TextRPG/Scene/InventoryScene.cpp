...
void InventoryScene::Render()
{
	...
	m_screen.Write(2, 5, L"·¹º§: " + to_wstring(player.GetExperience().level));
	m_screen.Write(2, 6, L"°æÇèÄ¡: " + to_wstring(player.GetExperience().GetRequiredExpForNextLevel()) + L"/" + to_wstring(player.GetExperience().currentExp));
	m_screen.Write(2, 7, L"°ñµå: " + to_wstring(player.GetGold().amount));
...
void InventoryScene::HandleUnequipCommand(const wstring& cmd)
{
	...
	if (index == 0)
	{
		...
		{
			m_textPrompt.Enqueue({ L"½Ã½ºÅÛ : ¾ÆÀÌÅÛÀ» ÀåÂøÇÏ°í ÀÖÁö ¾Ê½À´Ï´Ù." });
			EnableInventoryMenu();
			m_currentSceneState = EInventorySceneState::Default;
			return;
		}
